...
export default function ProductDetail({ product, lang }: ProductDetailProps) {
    ...
    // Compute unique colors and sizes from variants attributes
    const availableColors = Array.from(new Set(variants.map((v: any) => v.attributes?.color).filter(Boolean))) as string[];
    const availableSizes = Array.from(new Set(variants.map((v: any) => v.attributes?.size).filter(Boolean))) as string[];
    ...
    const currentVariant = variants.find((v: any) => {
        const matchColor = !availableColors.length || v.attributes?.color === selectedColor;
        const matchSize = !availableSizes.length || v.attributes?.size === selectedSize;
        ...
        <>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
                ...
                <div className="lg:col-span-7 space-y-4">
                    <div className="relative aspect-square rounded-3xl overflow-hidden bg-gray-50 border border-gray-100 shadow-sm group p-8 lg:p-12">
                        <Image
                            src={mainImageUrl}
                            alt={name}
                            fill
                ...
                {/* Info Section */}
                <div className="lg:col-span-5 flex flex-col pt-4 lg:pt-0 lg:sticky lg:top-24">
                    <div className="mb-8">
                        <div className="flex items-start justify-between gap-4 mb-2">
                            <h1 className="text-3xl lg:text-4xl font-black text-[#003366] leading-[1.1] tracking-tight decoration-[#FF6600]/20">
                                {name}
                            </h1>
                        </div>
                        ...
                        <div className="flex items-center gap-4 mt-6">
                            <div className="flex flex-col">
                                ...
                                <span className="text-4xl lg:text-5xl font-black text-[#003366] italic tracking-tighter leading-none">
                                    {finalPrice.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€
                                </span>
                            </div>
                            <div className="flex flex-col border-l border-gray-100 pl-4">
                                <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{dict.product.stock.inclVat}</span>
                                <span className="text-[10px] text-[#0072f5] font-black underline cursor-pointer hover:text-[#005bb5] transition-colors">{dict.product.stock.shippingCalc}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex gap-4">
                                ...
                                <button
                                    ...
                                    disabled={stock === 0}
                                    className={`flex-grow flex items-center justify-center gap-3 bg-[#FF6600] text-white rounded-2xl px-8 py-4 font-black text-lg transition-all shadow-lg shadow-[#FF6600]/20 active:scale-95 duration-200 uppercase tracking-tight ${stock === 0 ? 'opacity-50 cursor-not-allowed grayscale' : 'hover:bg-[#e65c00] hover:-translate-y-0.5'}`}
                                >
                            ...
                            <button
                                ...
                                disabled={stock === 0}
                                className={`w-full bg-[#003366] text-white rounded-2xl px-8 py-4 font-black text-lg transition-all shadow-lg shadow-[#003366]/20 active:scale-95 duration-200 uppercase tracking-tight ${stock === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#002244] hover:-translate-y-0.5'}`}
                            >
            ...
            <div className="mt-12 lg:mt-24 max-w-4xl">
                <div className="inline-block border-b-4 border-[#FF6600] pb-2 mb-8 lg:mb-12">
                    <h2 className="text-2xl lg:text-3xl font-black text-[#003366] uppercase tracking-tighter">{dict.product.description}</h2>
                </div>

                <div className="prose prose-lg max-w-none text-gray-600 leading-relaxed font-medium prose-headings:font-black prose-headings:text-[#003366] prose-p:mb-4 prose-ul:list-disc prose-ul:pl-4 prose-li:mb-1">
                    {description ? (
